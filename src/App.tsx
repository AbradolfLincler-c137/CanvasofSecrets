...
import { AuthProvider, useAuth } from './context/VitraAuthContext';

const AppContent: React.FC = () => {
  const { isIdentified } = useAuth();

  return (
    <div className="min-h-screen flex bg-surface selection:bg-secondary/20">
      {/* Paper Grain Overlay */}
      <div className="fixed inset-0 paper-grain z-[100] pointer-events-none" />
      
      {/* If not identified, show the Access Gate as a full-screen overlay */}
      {!isIdentified && <AccessGate />}

      {/* Main Layout - only interactable if identified */}
      <Sidebar />
